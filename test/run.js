...

{
  ...
  assertContains('worker: paginated list canonical keeps page', src, "if (!search && page > 1) canonicalParams.set('page', String(page))");
  assertContains('worker: language list canonical keeps lang', src, "if (!search && lang) canonicalParams.set('lang', lang)");
  ...
  assertContains('worker: short links skip locale redirect', src, "return path === '/' || path === '/about' || path === '/repos' || /^\\/repo\\//.test(path)");
  ...
  assertContains('worker: canonical repo detail path helper', src, 'function repoDetailPath');
  assertContains('worker: list links to canonical repo path', src, 'const repoDetailUrl = repoDetailPath(repo.full_name, langPrefix)');
  assertContains('worker: related links to canonical repo path', src, 'href="${repoDetailPath(r.full_name, langPrefix)}"');
  ...
  assertContains('worker: sitemap list counts query', src, 'async function getSitemapListCounts');
  assertContains('worker: sitemap category date canonical', src, 'repos?category=${encodeURIComponent(category)}&date=${encodeURIComponent(row.crawl_date)}');
  assertContains('worker: sitemap paginated list entries', src, 'baseUrl}&page=${page}');
  ...
  assertContains('worker: r id uses x robots tag',   src, 'function withRobotsHeader');
